...
assign_vr_cost = function(this_usage, rate_card, conn, run = 1) {
  ...
  # rate_card %>% filter(is.na(Request.Type))
  
  ##############################################################################
  ### INIT_CHARGE
  # temp = data[data$CTRB_BILLING %in% "Y" & data$CTRB_UNIQ %in% "Y" & data$INIT_CHARGE %in% "Y",]
  
  rate_card$INIT_CHARGE = sapply(rate_card$Request.Type, function(x) if(!is.na(x) & x %in% "Unique") T else F)
  rate_card$CTRB_UNIQ = sapply(rate_card$Request.Type, function(x) if(!is.na(x) & x %in% "Unique") T else F)
  
  ### ACCESS
  # temp = data[data$CTRB_BILLING %in% "Y" & data$CTRB_AC %in% "Y" & data$BO_OPTIMIZED %in% "N",]
  
  rate_card$CTRB_AC = sapply(rate_card$Request.Type, function(x) if(!is.na(x) & x %in% "Access") T else F)
  rate_card = rate_card %>% mutate(BO_OPTIMIZED = F)
  # rate_card$BO_OPTIMIZED = sapply(rate_card$Request.Type, function(x) if(!is.na(x) & x %in% "Access") F else T)
  
  ### MAINTAINED
  # temp = data[data$CTRB_BILLING %in% "Y" & data$CTRB_MAIN %in% "Y" & data$INIT_CHARGE %in% "Y",]
  
  rate_card$CTRB_MAIN = sapply(rate_card$Request.Type, function(x) if(!is.na(x) & x %in% "Maintained") T else F)
  rate_card = rate_card %>% mutate(INIT_CHARGE = ifelse(Request.Type %in% "Maintained", T, INIT_CHARGE))
  # rate_card$INIT_CHARGE = sapply(rate_card$Request.Type, function(x) if(!is.na(x) & x %in% "Maintained") T else F)
  
  ### BBO_OPTIMIZED
  # temp = data[data$CTRB_BILLING %in% "Y" & data$BO_OPTIMIZED %in% "Y",]
  ...
  ### if this is still required, now it has to be reversed to create a FALSE scenario
  ...
  ### rate card add an extra for the INIT that also have CTRB_AC as 1
  rate_card = rate_card %>% filter(Request.Type %in% c("Unique")) %>% mutate(CTRB_AC = T) %>%
    plyr::rbind.fill(rate_card)
  
  ### rate card add an extra for the ACCESS that also have CTRB_UNIQ as 1
  rate_card = rate_card %>% filter(Request.Type %in% c("Access")) %>% mutate(CTRB_UNIQ = T) %>%
    plyr::rbind.fill(rate_card)
  ...
  ### making an average effective rate card so that the hit rate for Sched INIT's are not the cost of the whole tier
  ...
  rate_card = rate_card[!duplicated(rate_card[,c("BILLABLE_PRODUCT","INIT_CHARGE","CTRB_MAIN","BO_OPTIMIZED","CTRB_UNIQ","CTRB_AC")]),]
  rate_card
  ...
  table(this_usage$BILLABLE_PRODUCT, this_usage$CTRB_MAIN)

  ### if there is no value in CTRB_MAIN, then fill it in, it must be coming from sched or bval
  this_usage = this_usage %>% mutate(CTRB_MAIN = ifelse(is.na(CTRB_MAIN),0,CTRB_MAIN))
  ...
  df = df %>% mutate(INIT_CHARGE = ifelse(x,0,1))
  this_usage = this_usage %>% filter(!is.na(INIT_CHARGE)) %>% plyr::rbind.fill(df)
    # mutate(CTRB_MAIN = 0)

  table(this_usage$CTRB_MAIN)
  
  ### ok, let's figure out what's going on with SecMastr
  ...
  this_usage$List.Price %>% sum(na.rm = T)
  # ?sum
  table(this_usage$BILLABLE_PRODUCT, this_usage$Request.Type, useNA = "ifany")
  
  ### inspecting US GOVT
  # this_usage %>% filter(CTRB_UNIQ %in% T, INIT_CHARGE %in% T, BILLABLE_PRODUCT %in% "USGovt History")
  # this_usage %>% filter(CTRB_MAIN %in% T, BILLABLE_PRODUCT %in% "USGovt History")
  # table(this_usage$BILLABLE_PRODUCT, this_usage$CTRB_MAIN)

  this_usage = this_usage %>% mutate(
    # List.Price = ifelse(is.na(List.Price), .01, List.Price),
    cost = List.Price * factor
    )
  ...
  this_usage$cost[this_usage$CTRB_BILLING == 0] = 0

  ### this seems key, it must have an init or access charge...
  this_usage$cost[this_usage$INIT_CHARGE == 0 & this_usage$CTRB_AC == 0] = 0

  table(is.na(this_usage$cost))

  this_usage[is.na(this_usage$cost) & this_usage$INIT_CHARGE == 1,"cost"] = 1
  this_usage[is.na(this_usage$cost) & this_usage$INIT_CHARGE == 0,"cost"] = .01
